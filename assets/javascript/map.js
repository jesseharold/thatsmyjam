...
//function to create map after user has logged in via instagram 
function createMap(friendsListArray){

    //TEST DATA - current user info
    // var currentLocation = {lat: 34.0554665, lng: -118.30951240000002};

    //set user data
    var friendsList = friendsListArray;  //["4147363198", "4147363198", "4149753642"];
        ...
        latLngToAddress(restaurant.lat, restaurant.lng, function(result){
            ...
            for (var j = 0; j < allReviews.length; j++){  //loop through all the reviews for the restaurant
                if (friendsList.indexOf(allReviews[j].author) >= 0){  //if the reviewer is on the friendsList then proceed...  
                    ...
                    markerHTML = markerHTML + "<p class='review-author'>" + localCopyUsers[allReviews[j].author].name + " says: </p>";  //look up the author's name by user id
    ...
    } //end function setAddressSuggest()
